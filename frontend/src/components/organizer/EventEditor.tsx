...
function EditTicketType({ event, ticket, open, onOpenChange, onChange }: { event: OrganizerEvent; ticket: OrganizerTicketType; open: boolean; onOpenChange: (open: boolean) => void; onChange: () => Promise<void> }) {
  ...
  async function submit(eventSubmit: FormEvent) { eventSubmit.preventDefault(); setBusy(true); const { error } = await api.PATCH("/api/v1/organizer/events/{eventId}/ticket-types/{ticketTypeId}", { params: { path: { eventId: event.id, ticketTypeId: ticket.id } }, body: { name: form.name, description: form.description || null, priceCents: Math.round(Number(form.price.replace(",", ".")) * 100), maxPerOrder: Number(form.maxPerOrder), saleStartsAt: form.saleStartsAt ? new Date(form.saleStartsAt).toISOString() : null, saleEndsAt: form.saleEndsAt ? new Date(form.saleEndsAt).toISOString() : null } }); if (error) toast.error(problemMessage(error)); else { toast.success("Ingresso atualizado."); onOpenChange(false); await onChange(); } setBusy(false); }
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="sm:max-w-lg"><form onSubmit={submit}><DialogHeader><DialogTitle>Editar tipo de ingresso</DialogTitle><DialogDescription>A capacidade é ajustada separadamente para preservar o estoque.</DialogDescription></DialogHeader><div className="mt-5 grid gap-4 sm:grid-cols-2"><div className="sm:col-span-2"><EditorField label="Nome" id={`edit-ticket-name-${ticket.id}`} value={form.name} onChange={(value) => setForm((current) => ({ ...current, name: value }))} /></div><div className="sm:col-span-2"><EditorField label="Descrição" id={`edit-ticket-description-${ticket.id}`} value={form.description} onChange={(value) => setForm((current) => ({ ...current, description: value }))} required={false} /></div><EditorField label="Preço (R$)" id={`edit-ticket-price-${ticket.id}`} value={form.price} onChange={(value) => setForm((current) => ({ ...current, price: value }))} /><EditorField label="Máx. por pedido" id={`edit-ticket-max-${ticket.id}`} type="number" value={form.maxPerOrder} onChange={(value) => setForm((current) => ({ ...current, maxPerOrder: value }))} /><EditorField label="Início das vendas" id={`edit-ticket-start-${ticket.id}`} type="datetime-local" value={form.saleStartsAt} onChange={(value) => setForm((current) => ({ ...current, saleStartsAt: value }))} required={false} /><EditorField label="Fim das vendas" id={`edit-ticket-end-${ticket.id}`} type="datetime-local" value={form.saleEndsAt} onChange={(value) => setForm((current) => ({ ...current, saleEndsAt: value }))} required={false} /></div><DialogFooter className="mt-5"><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Cancelar</Button><Button type="submit" disabled={busy}>{busy ? "Salvando..." : "Salvar ingresso"}</Button></DialogFooter></form></DialogContent></Dialog>;
}
...
function CreateTicketType({ event, onChange }: { event: OrganizerEvent; onChange: () => Promise<void> }) {
  ...
  async function submit(eventSubmit: FormEvent) { eventSubmit.preventDefault(); setBusy(true); const { error } = await api.POST("/api/v1/organizer/events/{id}/ticket-types", { params: { path: { id: event.id } }, body: { name: form.name, description: form.description || undefined, priceCents: Math.round(Number(form.price.replace(",", ".")) * 100), capacity: Number(form.capacity), maxPerOrder: Number(form.maxPerOrder), saleStartsAt: form.saleStartsAt ? new Date(form.saleStartsAt).toISOString() : undefined, saleEndsAt: form.saleEndsAt ? new Date(form.saleEndsAt).toISOString() : undefined } }); if (error) toast.error(problemMessage(error)); else { toast.success("Tipo de ingresso criado."); setForm(initial); await onChange(); } setBusy(false); }
  return <Card className="h-fit"><CardHeader><CardTitle className="text-base">Novo tipo</CardTitle></CardHeader><CardContent><form onSubmit={submit} className="space-y-4"><EditorField label="Nome" id="ticket-name" value={form.name} onChange={(value) => setForm((current) => ({ ...current, name: value }))} /><EditorField label="Descrição" id="ticket-description" value={form.description} onChange={(value) => setForm((current) => ({ ...current, description: value }))} required={false} /><div className="grid grid-cols-2 gap-3"><EditorField label="Preço (R$)" id="ticket-price" value={form.price} onChange={(value) => setForm((current) => ({ ...current, price: value }))} /><EditorField label="Capacidade" id="ticket-capacity" type="number" value={form.capacity} onChange={(value) => setForm((current) => ({ ...current, capacity: value }))} /><EditorField label="Máx. por pedido" id="ticket-max" type="number" value={form.maxPerOrder} onChange={(value) => setForm((current) => ({ ...current, maxPerOrder: value }))} /><EditorField label="Início das vendas" id="ticket-sale-start" type="datetime-local" value={form.saleStartsAt} onChange={(value) => setForm((current) => ({ ...current, saleStartsAt: value }))} required={false} /><EditorField label="Fim das vendas" id="ticket-sale-end" type="datetime-local" value={form.saleEndsAt} onChange={(value) => setForm((current) => ({ ...current, saleEndsAt: value }))} required={false} /></div><Button type="submit" className="w-full" disabled={busy}><Plus /> {busy ? "Criando..." : "Criar ingresso"}</Button></form></CardContent></Card>;
}
...
function MediaPanel({ event, editable, onChange }: { event: OrganizerEvent; editable: boolean; onChange: () => Promise<void> }) {
  ...
  return <div className="space-y-6"><Card><CardHeader><CardTitle className="text-base">Capa do evento</CardTitle></CardHeader><CardContent>{cover?.url ? <div className="relative aspect-[16/6] overflow-hidden rounded-xl"><Image src={cover.url} alt="Capa atual" fill className="object-cover" sizes="100vw" />{editable && <Button variant="destructive" size="icon" className="absolute right-3 top-3" onClick={() => remove(cover.id)}><Trash2 /></Button>}</div> : <UploadDropzone label="Enviar capa" disabled={!editable || busy} onFile={(file) => upload(file, "cover")} />}</CardContent></Card><Card><CardHeader><CardTitle className="text-base">Galeria <Badge variant="secondary">{gallery.length}/6</Badge></CardTitle></CardHeader><CardContent><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{gallery.map((image, index) => <div key={image.id} className="relative aspect-video overflow-hidden rounded-xl border border-white/8">{image.url && <Image src={image.url} alt={`Imagem ${index + 1}`} fill className="object-cover" sizes="33vw" />} {editable && <div className="absolute bottom-2 right-2 flex gap-1"><Button size="icon-xs" variant="secondary" onClick={() => move(index, -1)} disabled={index === 0}><ArrowUp /></Button><Button size="icon-xs" variant="secondary" onClick={() => move(index, 1)} disabled={index === gallery.length - 1}><ArrowDown /></Button><Button size="icon-xs" variant="destructive" onClick={() => remove(image.id)}><Trash2 /></Button></div>}</div>)}{gallery.length < 6 && <UploadDropzone label="Adicionar à galeria" disabled={!editable || busy} onFile={(file) => upload(file, "gallery")} />}</div></CardContent></Card></div>;
}
