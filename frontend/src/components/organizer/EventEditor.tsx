...
function CreateTicketType({ event, onChange }: { event: OrganizerEvent; onChange: () => Promise<void> }) {
  ...
  return <Card className="h-fit"><CardHeader><CardTitle className="text-base">Novo tipo</CardTitle></CardHeader><CardContent><form onSubmit={submit} className="space-y-4"><EditorField label="Nome" id="ticket-name" value={form.name} onChange={(value) => setForm((current) => ({ ...current, name: value }))} /><EditorField label="Descrição" id="ticket-description" value={form.description} onChange={(value) => setForm((current) => ({ ...current, description: value }))} required={false} /><div className="grid gap-3 sm:grid-cols-2"><EditorField label="Preço (R$)" id="ticket-price" value={form.price} onChange={(value) => setForm((current) => ({ ...current, price: value }))} /><EditorField label="Capacidade" id="ticket-capacity" type="number" value={form.capacity} onChange={(value) => setForm((current) => ({ ...current, capacity: value }))} /><EditorField label="Máx. por pedido" id="ticket-max" type="number" value={form.maxPerOrder} onChange={(value) => setForm((current) => ({ ...current, maxPerOrder: value }))} /><div className="sm:col-span-2"><DateTimeField label="Início das vendas" id="ticket-sale-start" value={form.saleStartsAt} onChange={(value) => setForm((current) => ({ ...current, saleStartsAt: value }))} required={false} /></div><div className="sm:col-span-2"><DateTimeField label="Fim das vendas" id="ticket-sale-end" value={form.saleEndsAt} onChange={(value) => setForm((current) => ({ ...current, saleEndsAt: value }))} required={false} /></div></div><Button type="submit" className="w-full" disabled={busy}><Plus /> {busy ? "Criando..." : "Criar ingresso"}</Button></form></CardContent></Card>;
}
