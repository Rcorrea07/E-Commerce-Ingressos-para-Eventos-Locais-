...
export function AnalyticsDashboard({ scope }: { scope: "organizer" | "admin" }) {
  ...
  return <div><PageHeader eyebrow={scope === "admin" ? "Pulso global" : "Seus eventos"} title={scope === "admin" ? "A plataforma agora" : "O ritmo da sua operação"} description="Dados úteis para decidir o próximo passo, sem ruído." /><div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{cards.map((item) => <MetricCard key={item.label} {...item} />)}</div><div className="mt-6 grid gap-6 xl:grid-cols-[.9fr_1.1fr]"><Card><CardHeader><CardTitle className="text-base">Funil de checkout</CardTitle></CardHeader><CardContent className="space-y-5"><FunnelLine label="Iniciados" value={data.funnel.started} total={data.funnel.started} /><FunnelLine label="Confirmados" value={data.funnel.confirmed} total={data.funnel.started} /><FunnelLine label="Abandonados" value={data.funnel.abandoned} total={data.funnel.started} /><div className="flex items-center justify-between rounded-xl bg-primary/7 p-4"><span className="text-sm text-muted-foreground">Conversão</span><strong className="text-xl text-primary">{(data.funnel.conversionRate * 100).toFixed(1)}%</strong></div></CardContent></Card><Card><CardHeader><CardTitle className="text-base">Ocupação por ingresso</CardTitle></CardHeader><CardContent className="space-y-4">{data.occupancy.length === 0 ? <p className="text-sm text-muted-foreground">Sem dados de ocupação ainda.</p> : data.occupancy.slice(0, 6).map((item) => <div key={item.ticketTypeId}><div className="mb-2 flex justify-between gap-4 text-sm"><span className="truncate text-white">{item.event} · {item.ticketType}</span><span className="text-muted-foreground">{(item.rate * 100).toFixed(0)}%</span></div><Progress value={item.rate * 100} /></div>)}</CardContent></Card></div><Card className="mt-6 overflow-hidden"><CardHeader><CardTitle className="text-base">Pedidos recentes</CardTitle></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Pedido</TableHead><TableHead>Evento</TableHead><TableHead>Cliente</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Total</TableHead></TableRow></TableHeader><TableBody>{data.recentOrders.length === 0 ? <TableRow><TableCell colSpan={5} className="py-10 text-center text-muted-foreground">Nenhum pedido recente.</TableCell></TableRow> : data.recentOrders.map((order) => <TableRow key={order.id}><TableCell><span className="font-mono text-xs">{order.publicId}</span><span className="mt-1 block text-[11px] text-muted-foreground">{formatDateTime(order.createdAt)}</span></TableCell><TableCell>{order.eventTitle}</TableCell><TableCell>{order.customerName}</TableCell><TableCell><Badge variant={order.status === "CONFIRMED" ? "default" : "secondary"}>{order.status}</Badge></TableCell><TableCell className="text-right font-medium">{formatMoney(order.totalCents)}</TableCell></TableRow>)}</TableBody></Table></div></CardContent></Card></div>;
}
